...
//   FaExternalLinkAlt, FaTimes, FaCheck, FaSearch
// } from 'react-icons/fa';
...
//   // 1. DISCOVERY: Fetch users of the opposite role automatically
//   useEffect(() => {
...
//       setMessages((prev) => prev.map(m => m._id === data.id ? { ...m, content: data.content } : m));
//     });

//     return () => { socket.current?.disconnect(); };
//   }, [user]);

//   // 3. HISTORY: Load previous messages when partner is selected
//   useEffect(() => {
//     if (selectedPartner) {
//       api.get(`/chat/history/${selectedPartner._id}`).then(res => {
...
//   return (
//     <div className="h-[calc(100vh-80px)] flex bg-white m-4 rounded-[40px] shadow-2xl overflow-hidden border border-slate-100">
      
//       {/* --- SIDEBAR: DIRECTORY --- */}
//       <div className="w-80 border-r border-slate-50 bg-slate-50/30 flex flex-col">
//         <div className="p-8">
//           <h2 className="text-xl font-black text-slate-900">Inbox</h2>
//           <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">
//             {user?.role === 'student' ? 'Verified Recruiters' : 'Potential Candidates'}
//           </p>
//         </div>
...
//               className={`p-4 rounded-3xl cursor-pointer transition-all flex items-center gap-4 ${selectedPartner?._id === p._id ? 'bg-white shadow-lg' : 'hover:bg-white/50'}`}
//             >
//               <div className="w-12 h-12 rounded-2xl bg-blue-600 flex items-center justify-center text-white font-black">
//                 {p.firstName[0]}
//               </div>
//               <div className="flex-1 min-w-0">
//                 <p className="font-bold text-sm text-slate-900 truncate">{p.firstName} {p.lastName}</p>
//                 <p className="text-[10px] text-slate-400 font-medium uppercase">{p.role}</p>
//               </div>
//             </div>
//           ))}
//           {partners.length === 0 && <p className="text-center text-slate-300 text-xs mt-10">No users found in directory.</p>}
//         </div>
//       </div>
...
//           <>
//             {/* Header */}
//             <div className="p-6 border-b border-slate-50 flex justify-between items-center shadow-sm z-10">
//               <div className="flex items-center gap-4">
//                 <div className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center text-slate-400">
//                   <FaUserCircle size={20}/>
//                 </div>
//                 <div>
//                   <h3 className="font-bold text-slate-900 text-sm">{selectedPartner.firstName} {selectedPartner.lastName}</h3>
//                   <button 
//                     onClick={() => navigate(`/profile/${selectedPartner._id}`)} 
//                     className="text-[10px] text-blue-600 font-black uppercase tracking-widest flex items-center gap-1 hover:underline"
//                   >
//                     View Professional Profile <FaExternalLinkAlt size={8}/>
//                   </button>
//                 </div>
//               </div>
//             </div>
...
//                     <div className={`relative max-w-[70%] p-4 rounded-3xl text-sm shadow-sm transition-all ${
//                       isMe 
...
//                             className="bg-transparent border-b border-white/30 outline-none text-white w-full py-1"
//                             autoFocus
//                           />
//                           <button onClick={() => saveEdit(msg._id)} className="text-green-400 p-1"><FaCheck /></button>
//                           <button onClick={() => setEditingId(null)} className="text-red-400 p-1"><FaTimes /></button>
//                         </div>
//                       ) : (
//                         <>
...
//                            <button onClick={() => startEditing(msg)} className="p-2 bg-white text-slate-400 rounded-xl shadow-sm hover:text-blue-600 transition"><FaEdit size={10}/></button>
//                            <button onClick={() => deleteMsg(msg._id)} className="p-2 bg-white text-slate-400 rounded-xl shadow-sm hover:text-red-600 transition"><FaTrash size={10}/></button>
//                         </div>
//                       )}
//                     </div>
//                   </div>
...
//               <form onSubmit={handleSend} className="flex gap-4 items-center bg-slate-50 p-2 rounded-[24px] ring-1 ring-slate-100 focus-within:ring-2 focus-within:ring-blue-600 transition shadow-inner">
//                 <input 
//                   value={input} 
//                   onChange={e => setInput(e.target.value)} 
//                   placeholder="Collaborate on a new opportunity..." 
//                   className="flex-1 bg-transparent px-6 py-3 rounded-2xl outline-none text-sm" 
//                 />
//                 <button 
//                   type="submit" 
//                   disabled={!input.trim()}
//                   className="p-4 bg-blue-600 text-white rounded-2xl shadow-lg hover:bg-blue-700 transition disabled:opacity-50 active:scale-95"
//                 >
//                   <FaPaperPlane />
//                 </button>
//               </form>
...
//             </div>
//             <h3 className="text-2xl font-black text-slate-900 tracking-tight">Professional Inbox</h3>
//             <p className="text-slate-400 text-sm max-w-xs leading-relaxed">Select a professional from the directory to start coordinating.</p>
//           </div>
//         )}
//       </div>
//     </div>
