...
//   FaCheckCircle, FaBell, FaClock, FaChartLine 
// } from 'react-icons/fa';
...
//   if (isLoading) return <div className="p-20 text-center animate-pulse text-slate-300 font-black">Syncing Database...</div>;

//   return (
//     <div className="p-10 max-w-[1600px] mx-auto space-y-10">
//       <header className="flex justify-between items-center">
//         <div>
//           <h1 className="text-4xl font-black text-slate-900 tracking-tight">Hiring Command</h1>
//           <p className="text-slate-500 font-medium">Real-time data as of {new Date().toLocaleTimeString()}</p>
//         </div>
//         <button 
//           onClick={() => navigate('/recruiter/post-job')}
//           className="bg-slate-900 text-white px-10 py-5 rounded-[24px] font-black shadow-xl hover:bg-blue-600 transition flex items-center gap-3"
//         >
//           <FaPlus /> Post a New Role
//         </button>
//       </header>
...
//         {[
//           { label: 'Active Roles', value: analytics?.activeJobs || 0, icon: FaBriefcase, color: 'text-blue-600', bg: 'bg-blue-50' },
//           { label: 'Total Applicants', value: analytics?.totalApplicants || 0, icon: FaUsers, color: 'text-indigo-600', bg: 'bg-indigo-50' },
//           { label: 'Avg. Fill Rate', value: '74%', icon: FaChartLine, color: 'text-green-600', bg: 'bg-green-50' }
//         ].map((s, i) => (
...
//         {/* Trend Chart & Jobs List */}
//         <div className="lg:col-span-8 space-y-10">
//           <div className="bg-white p-10 rounded-[40px] border border-slate-100 shadow-sm">
//              <h3 className="font-bold text-slate-900 mb-8">Applicant Volume (Real-time)</h3>
...
//                       <Area type="monotone" dataKey="apps" stroke="#4f46e5" strokeWidth={4} fill="#4f46e520" />
//                     </AreaChart>
//                   </ResponsiveContainer>
//                 ) : (
//                   <div className="h-full flex items-center justify-center text-slate-300 text-xs font-bold uppercase">No data to display yet.</div>
//                 )}
//              </div>
//           </div>
...
//                       <p className="font-black text-slate-900 text-lg">{job.title}</p>
//                       <p className="text-slate-400 text-xs font-medium">{job.location} • {job.type}</p>
//                     </td>
//                     <td className="px-8 py-8">
//                        <span className={`px-4 py-1.5 rounded-xl font-black text-[10px] uppercase tracking-widest ${job.status === 'filled' ? 'bg-amber-50 text-amber-600' : 'bg-green-50 text-green-600'}`}>
//                           {job.status}
//                        </span>
//                     </td>
//                     <td className="px-8 py-8 text-right flex justify-end gap-3 pt-10">
//                       <button onClick={() => toggleStatus.mutate({ id: job._id, status: job.status === 'open' ? 'filled' : 'open' })} className="p-4 bg-slate-50 text-slate-400 rounded-2xl hover:text-blue-600 transition"><FaCheckCircle/></button>
//                       <button onClick={() => { if(window.confirm("Delete job?")) deleteJob.mutate(job._id) }} className="p-4 bg-red-50 text-red-400 rounded-2xl hover:bg-red-600 hover:text-white transition"><FaTrash/></button>
//                     </td>
//                   </tr>
//                 ))}
//               </tbody>
...
//               <h3 className="font-black text-slate-900 mb-10 flex items-center gap-3"><FaBell className="text-indigo-600" /> Activity</h3>
//               <div className="space-y-10">
//                  {activity?.length > 0 ? activity.map((act: any, i: number) => (
//                    <div key={i} className="flex gap-5 relative">
//                       {i !== activity.length - 1 && <div className="absolute left-[20px] top-10 bottom-[-40px] w-px bg-slate-100"></div>}
//                       <div className="w-10 h-10 rounded-2xl bg-slate-900 flex items-center justify-center text-white font-black text-xs shrink-0">{act.user[0]}</div>
//                       <div>
//                         <p className="text-sm text-slate-600 leading-snug"><span className="font-black text-slate-900">{act.user}</span> {act.action} <span className="font-black text-indigo-600">{act.target}</span></p>
//                         <p className="text-[10px] text-slate-400 font-black uppercase mt-1 flex items-center gap-1"><FaClock size={8}/> {formatDistanceToNow(new Date(act.time))} ago</p>
//                       </div>
//                    </div>
//                  )) : <div className="py-20 text-center text-slate-300 text-xs font-bold uppercase tracking-widest italic">No interactions yet.</div>}
//               </div>
//            </div>
//         </div>
//       </div>
