...
//   FaCheckCircle, FaRocket, FaUserShield, FaBriefcase
// } from 'react-icons/fa'; 
// import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';
// import api from '../services/api';
// import { useAuth } from '../contexts/AuthContext';

// // Mock trend data to populate the visual context
...
//           <p className="text-slate-500 dark:text-slate-400 mt-1 font-medium italic">Welcome back, {user?.firstName}. Your profile is currently at 78% optimization.</p>
//         </div>
//         <button onClick={() => navigate('/jobs')} className="z-10 bg-primary text-white px-10 py-5 rounded-3xl font-black shadow-2xl shadow-primary/30 hover:scale-105 transition-all flex items-center gap-3">
//           Launch Job Search <FaRocket />
//         </button>
...
//           { label: 'Recruiter Views', value: analytics?.profileViews || 142, icon: FaChartLine, color: 'text-emerald-500', bg: 'bg-emerald-500/10' }
//         ].map((s, i) => (
//           <motion.div key={i} whileHover={{ y: -5 }} className="glass p-8 rounded-[35px] shadow-sm relative group overflow-hidden">
//              <div className="flex justify-between items-start relative z-10">
//                 <div className={`p-4 rounded-2xl ${s.bg} ${s.color}`}><s.icon size={20} /></div>
...
//                    {s.label === 'ATS Score' ? `${s.value}%` : s.value}
//                 </h3>
//              </div>
...
//                 <div>
//                    <h3 className="font-black text-slate-900 dark:text-white uppercase text-xs tracking-widest">Profile Integrity</h3>
//                    <p className="text-slate-400 text-sm mt-1">Completion Status</p>
//                 </div>
//                 <span className="text-primary font-black text-2xl">78%</span>
//              </div>
//              <div className="w-full h-4 bg-slate-100 dark:bg-white/5 rounded-full overflow-hidden">
//                 <motion.div 
//                   initial={{ width: 0 }} animate={{ width: '78%' }} transition={{ duration: 1.5, ease: "easeOut" }}
//                   className="h-full bg-gradient-to-r from-primary to-blue-400 rounded-full shadow-lg" 
//                 />
//              </div>
//              <div className="grid grid-cols-3 gap-4 mt-8">
//                 {['Verified Email', 'AI Resume', 'Portfolio'].map((tag) => (
//                   <div key={tag} className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-tighter bg-slate-50 dark:bg-white/5 px-3 py-2 rounded-xl border border-slate-100 dark:border-white/5">
//                     <FaCheckCircle className="text-green-500" /> {tag}
//                   </div>
//                 ))}
//              </div>
//           </div>
...
//           {/* VISIBILITY CHART */}
//           <div className="glass p-8 rounded-[40px] h-[350px]">
//              <h3 className="font-black text-slate-900 dark:text-white uppercase text-xs tracking-widest mb-8">Profile Visibility Trend</h3>
...
//               <h3 className="font-black text-slate-900 dark:text-white uppercase text-xs tracking-widest">Recommended for You</h3>
//               <Link to="/jobs" className="text-primary font-bold text-xs uppercase hover:underline">Marketplace <FaArrowRight className="inline ml-1" /></Link>
//             </div>
//             <div className="space-y-4">
//               {recommendedJobs?.map((job: any) => (
//                 <div key={job._id || job.id} className="flex items-center justify-between p-5 rounded-3xl bg-slate-50/50 dark:bg-white/5 hover:bg-white dark:hover:bg-white/10 transition-all border border-transparent hover:border-primary/10 group">
//                   <div className="flex items-center gap-5">
//                     <div className="w-14 h-14 bg-slate-900 text-white rounded-2xl flex items-center justify-center font-black text-xl group-hover:scale-110 transition-transform shadow-lg">{job.company[0]}</div>
//                     <div className="min-w-0">
//                       <h4 className="font-bold text-slate-900 dark:text-white truncate">{job.title}</h4>
//                       <p className="text-slate-400 text-sm truncate">{job.company} • {job.location}</p>
//                     </div>
//                   </div>
//                   <button onClick={() => navigate(`/jobs/${job._id || job.id}`)} className="px-6 py-2 bg-white dark:bg-slate-800 dark:text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-sm group-hover:bg-primary group-hover:text-white transition">Apply</button>
//                 </div>
//               ))}
//             </div>
//           </div>
...
//             <div className="relative z-10">
//               <FaMagic className="mb-6 text-primary group-hover:rotate-12 transition-transform" size={40} />
//               <h3 className="text-2xl font-black mb-4">AI Recruiter Assistant</h3>
//               <p className="text-slate-400 text-sm leading-relaxed mb-10 italic">
//                 "Based on your latest resume, you have an 85% match for Senior UI roles. I recommend practicing 'System Design' interview questions today."
//               </p>
//               <button onClick={() => navigate('/interview')} className="w-full bg-white text-slate-950 py-5 rounded-3xl font-black text-sm uppercase tracking-widest hover:bg-primary hover:text-white transition shadow-xl shadow-primary/20">
//                 Start Prep Session
//               </button>
//             </div>
//             <div className="absolute -bottom-20 -right-20 w-64 h-64 bg-primary/20 rounded-full blur-3xl group-hover:bg-primary/30 transition-colors"></div>
//           </div>
...
//                <FaClock className="text-primary" /> Live Feed
//              </h3>
//              <div className="space-y-10">
//                 {[
//                   { label: 'Applied to Google', time: '2h ago', icon: FaCheckCircle, color: 'text-green-500' },
//                   { label: 'AI Resume Scored 88%', time: '5h ago', icon: FaStar, color: 'text-amber-500' },
//                   { label: 'Joined Marketplace', time: '1d ago', icon: FaUserShield, color: 'text-blue-500' }
//                 ].map((act, i) => (
//                   <div key={i} className="flex gap-4 relative">
//                      {i !== 2 && <div className="absolute left-[13px] top-8 bottom-[-40px] w-px bg-slate-100 dark:bg-white/5"></div>}
//                      <div className={`w-7 h-7 rounded-full bg-slate-50 dark:bg-white/5 flex items-center justify-center ${act.color} text-[10px] shadow-sm border border-slate-100 dark:border-white/5 shrink-0 z-10`}>
//                         <act.icon />
//                      </div>
//                      <div>
//                        <p className="text-sm font-bold text-slate-900 dark:text-white leading-tight">{act.label}</p>
//                        <p className="text-[10px] text-slate-400 font-black uppercase mt-1">{act.time}</p>
//                      </div>
//                   </div>
//                 ))}
//              </div>
...
//              <h3 className="font-black text-slate-900 dark:text-white uppercase text-[10px] tracking-widest mb-4">Jump Back In</h3>
//              <div className="grid grid-cols-2 gap-4">
//                 <Link to="/resumes" className="flex flex-col items-center justify-center p-6 bg-slate-50 dark:bg-white/5 rounded-3xl hover:bg-primary/10 transition group">
//                    <FaFileAlt className="text-slate-400 group-hover:text-primary mb-3" size={20}/>
//                    <span className="text-[10px] font-black dark:text-white uppercase tracking-tighter">Editor</span>
//                 </Link>
//                 <Link to="/chat" className="flex flex-col items-center justify-center p-6 bg-slate-50 dark:bg-white/5 rounded-3xl hover:bg-primary/10 transition group">
//                    <FaBriefcase className="text-slate-400 group-hover:text-primary mb-3" size={20}/>
//                    <span className="text-[10px] font-black dark:text-white uppercase tracking-tighter">Messages</span>
//                 </Link>
//              </div>
//           </div>
//         </div>
