...
export const validateJobRole = async (jobTitle: string) => {
  ...
  if (title.length < 3 || nonsenseSequences.test(title) || !hasVowels || repeatingChars) {
    return { isValid: false, message: "Please enter a legitimate professional job title." };
  }

  const client = getGeminiClient();
  if (!client) return { isValid: true }; 
...
export const generateInterviewQuestions = async (jobTitle: string, jobDescription: string) => {
  const client = getGeminiClient();
  if (!client) return { questions: ["Describe your experience.", "Why you?"] };

  try {
    const model = client.getGenerativeModel({ model: "gemini-1.5-flash" });
    const prompt = `Generate 10 interview questions for a ${jobTitle} role. Context: ${jobDescription}. Return ONLY JSON: { "questions": ["q1", "q2", ...] }`;
    const result = await model.generateContent(prompt);
    return parseAIResponse(result.response.text());
  } catch (error) { return { questions: ["Describe your experience."] }; }
};

export const evaluateInterviewAnswer = async (question: string, answer: string, jobTitle: string) => {
  const client = getGeminiClient();
  if (!client) return { isRelevant: true, score: 70, feedback: "Good." };

  try {
    const model = client.getGenerativeModel({ model: "gemini-1.5-flash" });
    const prompt = `Evaluate: Q: "${question}", A: "${answer}" for ${jobTitle}. Return ONLY JSON: { "isRelevant": boolean, "score": number, "feedback": "string" }`;
    const result = await model.generateContent(prompt);
    return parseAIResponse(result.response.text());
  } catch (error) { return { isRelevant: true, score: 50, feedback: "Error." }; }
};

export const analyzeResumeContent = async (resumeData: any) => {
  const client = getGeminiClient();
  if (!client) return { score: 75, feedback: ["Good."] };
  try {
    const model = client.getGenerativeModel({ model: "gemini-1.5-flash" });
    const result = await model.generateContent(`Analyze resume. Return ONLY JSON: { "score": number, "feedback": string[], "missingKeywords": string[] }`);
    return parseAIResponse(result.response.text());
  } catch (error) { return { score: 60, feedback: ["Error."] }; }
};

/**
 * MISSING FUNCTION ADDED BELOW
 */
export const generateCoverLetter = async (resumeContent: any, jobDescription: string) => {
  const client = getGeminiClient();
  if (!client) return "Gemini API key missing. Could not generate cover letter.";

  try {
    const model = client.getGenerativeModel({ model: "gemini-1.5-flash" });
    const prompt = `Based on this resume content: ${JSON.stringify(resumeContent)} and this job description: ${jobDescription}, write a professional and convincing cover letter.`;
    const result = await model.generateContent(prompt);
